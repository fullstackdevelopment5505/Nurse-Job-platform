...
import * as historyDuck from "../../../store/ducks/history.duck";
...
const mapStateToProps = (state) => ({
  historys: state.history.historys,
...
const EnhancedTableToolbar = props => {
  ...
  function handleMultiRemoveClick(){
    enableLoading();
    setTimeout(() => {
      actions.deleteHistorys(selected)
        .then(res => {
          disableLoading();
          let {data} = res;
    ...
    >
      ...
      <div className={classes.actions}>
        {numSelected > 0 ? (
          ...
          <Tooltip title="New History">
            {/* <IconButton aria-label="AddCircleOutlineIcon" onClick={handleAddClick}>
              <AddCircleOutlineIcon />
            </IconButton> */}
             <Link to="/admin/history/create-edit"><IconButton aria-label="AddCircleOutlineIcon" onClick={handleAddClick}>
              <AddCircleOutlineIcon />
            </IconButton></Link>
          </Tooltip>
        )}
...
EnhancedTableToolbar.propTypes = {
  ...
  allHistorys:PropTypes.func,
  handleunSelect:PropTypes.func,
  setCurHistory:PropTypes.func
};
...
//   const [curdescription,setCurdescription]= React.useState("");
  ...
  useEffect(() => {
    setAllinfo(props.historys);
  }, [props])
  function handleEditClick(row){
    props.setCurHistory(row)
  }
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        ...
        selected={selected} 
        allHistorys={props.allHistorys}
        handleunSelect={handleunSelect}
        setCurHistory={props.setCurHistory}
        />
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">{row.nurse}</TableCell>
                      <TableCell align="center">{row.status}</TableCell>
                      <TableCell align="right">
                          <Link to='/admin/history/create-edit'onClick={()=>handleEditClick(row)}><IconButton aria-label="EditIcon"  style={{paddingTop:0,paddingBottom:0}}>
                            <EditIcon />
                          </IconButton> </Link>
