...
function Account(props) {
  ...
  useEffect(() => {
    ...
    if(props.curuser.category)
      setCategory(props.curuser.category || '');
    else
  ...
  const save = () => {
     if(email === '' || category === '' || first === '' || last=== '' || phone=== ''|| location=== ''){
  ...
  const handleSave=(result)=>{
    ...
    setTimeout(() => {
       if(id === ''){
      actions.addNurse(result).then(res=>{
        disableLoading();
        let {data} = res;
        if(!data.success){
    ...
    <div className="row" style={{backgroundColor: "white", padding: "40px 20px 40px 20px"}}>
      ...
      <div className="col-md-8">
        <div className="col-md-12">
          <span className="col-md-6 cold-sm-12" style={{fontSize: "1.275rem", fontWeight: 600 }}>Nurse Information</span>
          <div className="col-md-6 col-sm-12 pull-right">
            <Link to="/admin/nurse"><button className="btn btn-secondary pull-right">Cancel</button></Link>
